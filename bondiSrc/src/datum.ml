...
(* System calls *)
  ...
  StringMap.add "mytest" (["String"], "String")(
  StringMap.add "sysexec2string" (["String"], "String")(
 (* Networking operations *)
  ...
  StringMap.add "writeint" (["Socket"; "Int"], "Unit") (
  StringMap.add "readline" (["Socket"],"String") (
  StringMap.add "input" (["Socket"], "Char") (
...
(* Networking operations *)
  ...
  NStringMap.add "writeint" (["Socket"; "Int"], "Unit") (
  NStringMap.add "readline" (["Socket"],"String") (
  NStringMap.add "input" (["Socket"], "Char") (
...
let tryread = fun i ->
try input_line i with End_of_file -> "";;

(* FUNCTION REQUIRED FOR sysexec2list *)
...
let process2string s = 
let output = fst (process_output_to_list2 s) in
String.concat "\n" output;;
...
let eval_datum d = function
	  ...
	  | "gethost"  -> Host(Unix.gethostbyname s)
  	  | "mytest" -> String("hihi")
	  | "sysexec2string" -> String(process2string s)
...
(* Networking operations *)
	...
	| [Socket (s, i, (o,f), desc)] -> (
	  match d with
	  | "input"	-> if f then Char (input_char i)
				else Char ('a')
          | "readline" ->  if f
                                    then String(tryread i)
                                    else String("")
	  | "readall" -> if f
                                    then String(read i)
                                    else String("")
	...
	| [String m; Socket (s, i, (o,f), desc)] -> (
	 ...
	 | "write" -> if f
	  ...
	  | _ -> basicError (d^"is unknown string")	
	)
		
	| [Int m; Socket (s, i, (o,f), desc)] -> (
	  match d with
	  | "writeint" -> if f
				then (output_binary_int o m; flush o);
				Un
	 | _ -> basicError (d ^ " is unknown int")
        )
